...
#define WIFI_DATA_PROCESS_LMT           64              //串口数据处理缓存区大小,根据用户DP数据大小量定,必须大于24
#else
#define WIFI_UART_RECV_BUF_LMT          128             //串口数据接收缓存区大小,如MCU的RAM不够,可缩小

//请在此处选择合适的串口数据处理缓存大小（根据上面MCU固件升级包选择的大小和是否开启天气服务来选择开启多大的缓存）
#define WIFI_DATA_PROCESS_LMT           128             //串口数据处理缓存大小,如需MCU固件升级,若单包大小选择256,则缓存必须大于260,若开启天气服务,则需要更大
...
#define DPID_ATMOSPHERE_LAMP 109

#define DPID_BUZZER_SWITCH 110

#define DPID_VACUUM_STATE 111

#define DPID_ATMOSPHERE_STATE 112

#define DPID_FEED_PLAN_NUM 113

#define DPID_VACUUM_SWITCH 114
...
//唤宠物声音(可下发可上报)
//备注:在主页面中显示一个按键，点一次响一次，点一下，下发1；没有关的功能，在多份喂食和定时时可以做为设置项
#define DPID_VOICE 102
//唤宠物声音(可下发可上报)
//备注:录音状态： 0,：没有录音 1：存在录音
#define DPID_RECORDING_STATUS 103
...
//17-24位代表喂食类型
//1：自动 2：手动 3：按键
#define DPID_FEED_REPORT_JK 101



//锁键 (可下发可上报)
#define DPID_LOCK_KEY 104
//	干燥剂提醒时间 0-30天(可下发可上报)
#define DPID_REMIND_TIME 105
//	干燥剂剩余时间 0-30天(只上报)
#define DPID_REMAINING_TIME 106
//	干燥剂重置(可下发可上报)
#define DPID_DESICCANT_RESET 107
...
#define DPID_OFFLINE_FEED_RECORD 108
